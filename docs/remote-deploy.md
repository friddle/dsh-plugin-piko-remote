...
插件 `0.3.1`，piko 服务端 `https://clauded.friddle.me`（子域名模式）。
...
npm pack                         # 生成 dsh-plugin-piko-remote-0.3.1.tgz（含 bin/）
dsh plugin --profile piko add ~/dsh-plugin-piko-remote/dsh-plugin-piko-remote-0.3.1.tgz
