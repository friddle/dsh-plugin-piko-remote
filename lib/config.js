...
export const Config = z.object({
  ...
  endpoint: z
    ...
    .description(
      '固定 endpoint 名（autoExpose 与 remote_expose 都用它），留空则每次启动随机。' +
        '要配合 --trusted-host 时必须有固定名字，因为 --trusted-host 不支持通配符。',
    ),
  defaultTtlMinutes: z
    .natural()
  ...
  basicAuthUser: z
    .string()
    .default(DEFAULTS.basicAuthUser)
    .description('固定 Basic Auth 用户名；留空则每次随机生成。'),
  basicAuthPass: z
    .string()
    .role('secret')
    .default(DEFAULTS.basicAuthPass)
    .description('固定 Basic Auth 密码；留空则每次随机生成。设置后不会再随机。'),
  urlMode: z
    .string()
  ...
  credentialsFile: z
    .string()
    .default(DEFAULTS.credentialsFile)
    .description('autoExpose 成功后把 {remoteUrl, authUser, authPass} 以 0600 权限写到这个文件；留空则不写。'),
})
...
export function resolveConfig(raw = {}) {
  ...
  const cleaned = {
    ...
    endpoint: typeof merged.endpoint === 'string' ? merged.endpoint.trim() : DEFAULTS.endpoint,
    ...
    basicAuthUser: typeof merged.basicAuthUser === 'string' ? merged.basicAuthUser : DEFAULTS.basicAuthUser,
    basicAuthPass: typeof merged.basicAuthPass === 'string' ? merged.basicAuthPass : DEFAULTS.basicAuthPass,
    ...
    credentialsFile:
      typeof merged.credentialsFile === 'string' ? merged.credentialsFile : DEFAULTS.credentialsFile,
  }
