...
export function apply(ctx, config) {
  ...
  function maybeAutoExpose() {
    ...
    // on disk, and this URL is the only thing guarding the port. An operator
    // who needs them names a credentialsFile instead.
    void supervisor.expose({ port: webPort, ttlMinutes: settings.defaultTtlMinutes }).then(
      async (tunnel) => {
        log(`[piko-remote] auto-exposed the DSH Web UI on endpoint ${tunnel.endpoint}`)
        if (settings.credentialsFile === '') return
        try {
          const written = await writeCredentialsFile(settings.credentialsFile, tunnel)
          log(`[piko-remote] tunnel credentials written to ${written}`)
        } catch (error) {
          warn(`[piko-remote] could not write ${settings.credentialsFile}: ${error.message}`)
        }
      },
